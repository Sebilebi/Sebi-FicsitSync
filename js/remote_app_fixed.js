...
function switchMainMode(modeId) {
  ...
  views.forEach(v => {
    ...
    if (v.btn) {
      const isActive = v.id === modeId;
      v.btn.classList.toggle("active", isActive);
      v.btn.style.color = isActive ? "#000" : "var(--text-main)";
      
      if (isActive) {
        const slider = document.getElementById("nav-slider-indicator");
        if (slider) {
          slider.style.width = v.btn.offsetWidth + "px";
          slider.style.left = v.btn.offsetLeft + "px";
          slider.style.clipPath = v.btn.style.clipPath;
        }
      }
    }
  });
...
function initBlueprintPanZoom() {
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    if (Math.hypot(e.clientX - mouseStartX, e.clientY - mouseStartY) > 5) window.bpHasDragged = true;
  ...
  window.addEventListener('mouseup', () => {
    if (isDragging) {
      ...
      setTimeout(() => { window.bpHasDragged = false; }, 50);
    }
  });
...
function renderSchematics(branchId) {
  ...
  if (activeOnlyFilter && window.liveMetrics?.global) {
    items = items.filter(item => {
      ...
      return metric && (metric.production_nominal > 0 || metric.storage_count > 0 || metric.machines_count > 0 || metric.consumption_nominal > 0);
    });
  }
  ...
  function renderRow(item, index, side) {
    ...
    const zoneColor = linkedZone ? '#FA9549' : '#eab308';
    ...
    if (side === 'left') {
      return `
        <g transform="translate(20, ${y})">
          <!-- Entrada Tren Insumo -->
          <line x1="-15" y1="27" x2="10" y2="27" stroke="#FA9549" stroke-width="2" stroke-dasharray="3 2" />
          <rect x="-15" y="20" width="14" height="14" rx="2" fill="#0369a1" />
          <text x="-8" y="30" font-size="8" text-anchor="middle" fill="#ffffff">🚂</text>

          <!-- 1. Estación Entrada -->
          <g transform="translate(10, 0)">
            <rect x="0" y="0" width="85" height="54" rx="5" fill="#0c1829" stroke="#FA9549" stroke-width="1.5" />
            <rect x="2" y="2" width="81" height="11" rx="3" fill="#FA9549" />
            <text x="42.5" y="10" text-anchor="middle" font-size="6.5" font-weight="900" fill="#ffffff">ESTACIÓN ENTRADA</text>
            <text x="42.5" y="22" text-anchor="middle" font-size="5.5" font-weight="bold" fill="#FA9549">DESCARGA INSUMO</text>
          ...
          <!-- Conexión a Fábrica -->
          <line x1="95" y1="27" x2="118" y2="27" stroke="#FA9549" stroke-width="3" stroke-dasharray="${item.liquid ? '0' : '3 2'}" />
          ...
          <g transform="translate(118, 0)" class="bp-node-clickable" onclick="window.openItemModal('${item.id}')" title="Clic para ver ficha técnica de ${item.name}">
            <rect x="6" y="-6" width="5" height="6" fill="#5B5B5B" />
            <rect x="14" y="-9" width="5" height="9" fill="#5B5B5B" />
            <rect x="0" y="0" width="88" height="54" rx="5" fill="#2C2C2C" stroke="${isProducing ? '#10b981' : '#475569'}" stroke-width="${isProducing ? '2' : '1.5'}" />
            <text x="44" y="10" text-anchor="middle" font-size="6.5" font-weight="900" fill="${isProducing ? '#34d399' : '#A3A3A3'}">${machineType.substring(0, 16)}</text>
            <text x="44" y="19" text-anchor="middle" font-size="6.5" font-weight="bold" fill="#FFFFFF">${item.name.substring(0, 14)}</text>
            <rect x="6" y="23" width="26" height="26" rx="3" fill="#2C2C2C" stroke="#5B5B5B" stroke-width="1" />
            ...
            <text x="59" y="32" text-anchor="middle" font-size="5.5" font-weight="900" fill="${isProducing ? '#34d399' : '#A3A3A3'}">${rateText}</text>
          ...
          <g transform="translate(206, 0)">
            ...
            <text x="16" y="37" text-anchor="middle" font-size="5" font-weight="bold" fill="#A3A3A3">Tractor</text>
          </g>

          <!-- 3. Almacén Local -->
          <g transform="translate(238, 0)" class="bp-node-clickable" onclick="window.openItemModal('${item.id}')">
            <rect x="0" y="0" width="80" height="54" rx="5" fill="#2C2C2C" stroke="${itemColor}" stroke-width="2" />
            ...
            <rect x="28" y="23" width="24" height="24" rx="3" fill="#2C2C2C" stroke="${itemColor}" stroke-width="1" />
            <image href="icons/${item.id}.png" x="29" y="24" width="22" height="22" preserveAspectRatio="xMidYMid meet" />
          </g>

          <!-- Conexión a Salida -->
          <line x1="318" y1="27" x2="342" y2="27" stroke="#ea580c" stroke-width="3" stroke-dasharray="${item.liquid ? '0' : '2 2'}" />
      ...
      return `
        <g transform="translate(1310, ${y})">
          ...
          <!-- Cinta a Entrada -->
          <line x1="312" y1="27" x2="336" y2="27" stroke="#FA9549" stroke-width="3" stroke-dasharray="2 2" />
          ...
          <line x1="421" y1="27" x2="446" y2="27" stroke="#FA9549" stroke-width="2" stroke-dasharray="3 2" />
  ...
  const mallHtml = `
    ...
    <g transform="translate(${mallX}, ${mallY})">
      <rect x="0" y="0" width="${mallW}" height="${mallH}" rx="10" fill="#0b1329" stroke="#FA9549" stroke-width="2.5" filter="drop-shadow(0 8px 24px rgba(0,0,0,0.6))" />
      <rect x="0" y="0" width="${mallW}" height="42" rx="8" fill="#0f1f38" />
      <text x="${mallW / 2}" y="26" text-anchor="middle" font-size="13" font-weight="900" fill="#FA9549" letter-spacing="1">
      ...
      <g transform="translate(20, 100)">
        <text x="0" y="10" font-size="8" font-weight="900" fill="#A3A3A3">CONTENEDORES DISPONIBLES EN ESTA RAMA:</text>
          ...
          ${cat.items.map((item, idx) => {
            ...
            return `
              <g transform="translate(${col * 33}, ${row * 34})" class="bp-node-clickable" onclick="window.openItemModal('${item.id}')" title="${item.name}: ${isProd ? '+' + Math.round(metric.production_nominal) + '/m' : 'Buffer'}">
                <rect x="0" y="0" width="28" height="28" rx="4" fill="#2C2C2C" stroke="${isProd ? '#10b981' : (item.color || '#FA9549')}" stroke-width="${isProd ? '2' : '1.5'}" />
  ...
  stage.innerHTML = `
    ...
    <g>
      <rect x="540" y="${mallCenterY - 30}" width="24" height="14" rx="3" fill="#2C2C2C" stroke="#ef4444" stroke-width="1.5" />
      <text x="552" y="${mallCenterY - 20}" font-size="9" text-anchor="middle" fill="#ffffff">🚂</text>

      <rect x="1170" y="${mallCenterY - 30}" width="24" height="14" rx="3" fill="#2C2C2C" stroke="#ef4444" stroke-width="1.5" />
...
function updateSyncCountdownUI() {
  ...
  const sessName = window.liveMetrics?.session?.sessionName || "FICSIT INC.";
  const brandTitle = document.querySelector(".brand-title");
  if (brandTitle && brandTitle.innerText !== sessName) brandTitle.innerText = sessName.toUpperCase();
  const saveName = window.liveMetrics?.active_save_file || window.liveMetrics?.session?.sessionName || 'Satisfactory';
  if (window.isSyncPaused) {
    badge.innerHTML = `<span style="color:#aaa;">\u23F8 ${saveName} &bull; Pausado (${timeStr})</span>`;
  } else {
    badge.innerHTML = `<span style="color:#22c55e;">\u25CF</span> <strong>${saveName}</strong> &bull; Autosync: <strong>${timeStr}</strong>`;
  }
}
...
function applyMapData(bldData, zonesData, nodes) {
  ...
  if (document.getElementById('scim_tab_pane_collectibles')?.classList.contains('active')) {
...
function openMachineInGameModal(b) {
      ...
      inputsEl.innerHTML = b.inputs.map(inp => `
        <div class="scim-m-slot-row">
          ...
          <div>
            ...
            <div style="font-size: 10px; color: #A3A3A3;">${Math.round(inp.rate * 10) / 10}/min</div>
          </div>
        </div>
      `).join('');
    } else {
      inputsEl.innerHTML = `<div style="color: #A3A3A3; font-size: 11px; text-align: center; padding: 8px;">Sin insumos requeridos</div>`;
    }
  }
  ...
  if (outputsEl) {
    if (b.outputs && b.outputs.length > 0) {
      ...
      outputsEl.innerHTML = `<div style="color: #A3A3A3; font-size: 11px; text-align: center; padding: 8px;">Sin producto asignado</div>`;
    }
  }
...
function switchScimSidebarTab(tabName) {
  const tabs = [
    ...
    { id: 'collectibles', btnId: 'btn_scim_tab_collectibles', paneId: 'scim_tab_pane_collectibles' }
  ];
...
function renderCollectiblesList(subTab) {
  ...
  // Map sub-tab id to data tab
  const tabKey = subTab === 'collectibles_items' ? 'collectibles' : subTab;
  const tabData = MAP_COLLECTIBLE_TABS[tabKey];
  if (!tabData) { container.innerHTML = '<p style="color: var(--text-muted); font-size: 12px;">No hay datos.</p>'; return; }
  ...
  for (const group of tabData.groups) {
    // Group label header (e.g. "DROP-PODS", "CONSUMIBLES")
    const gl = group.groupLabelEs || group.groupLabel || null;
    if (gl && gl !== lastGroupLabel) {
      html += `<div class="collectible-group-label">${gl.toUpperCase()}</div>`;
      lastGroupLabel = gl;
    }
    ...
    const pct = total > 0 ? Math.round(collected / total * 100) : 0;
          ...
          html += `
        <label class="collectible-item-row ${isActive ? "active" : ""}" style="display:flex; align-items:center; gap:10px; cursor:pointer;">
          <input type="checkbox" ${isActive ? "checked" : ""} onchange="toggleCollectibleGroup('${group.name.replace(/'/g, "\\'")}')" />
          <div class="collectible-item-icon" style="width:24px; height:24px; display:flex; align-items:center; justify-content:center;">
            ${group.icon ? `<img src="icons/${group.icon}" alt="${group.nameEs}" style="max-width:100%; max-height:100%;" />` : "<span>?</span>"}
          </div>
          <span class="collectible-item-name" style="flex:1; font-size:13px; font-weight:700; color:#eee;">${group.nameEs}</span>
          <span class="scim-res-counter" title="${collected} obtenidos de ${total} totales" style="font-size:11px; font-weight:800; color:#aaa;">
...
function editZone(zoneId) {
  ...
  if (document.getElementById('zone-color-input')) {
    document.getElementById('zone-color-input').value = zone.color || '#FA9549';
  }
    ...
    panel.innerHTML = `
      <svg viewBox="0 0 1700 ${svgHeight}" style="width: 1700px; height: ${svgHeight}px; user-select: none; background-color: var(--svg-bg); display: block;">
        ...
        <g transform="translate(670, ${Math.max(20, Math.floor(svgHeight / 2) - 130)})">
          ...
          <rect x="0" y="0" width="320" height="40" rx="8" fill="#2C2C2C" />
          <text x="160" y="25" text-anchor="middle" font-size="13" font-weight="900" fill="#ffffff">🏛️ MALL CENTRAL</text>
          <text x="160" y="56" text-anchor="middle" font-size="10" font-weight="bold" fill="#FA9549">[ MATRIZ DE RECOGIDA ]</text>
        </g>
  ...
  if (side === 'left') {
    return `
      <g transform="translate(10, ${y})">
        <g transform="translate(10, 0)">
          <rect x="0" y="0" width="80" height="52" rx="4" fill="var(--bg-card)" stroke="#FA9549" stroke-width="1.5" />
          <rect x="2" y="2" width="76" height="10" rx="2" fill="#FA9549" />
          <text x="40" y="9" text-anchor="middle" font-size="6" font-weight="900" fill="#ffffff">ENTRADA</text>
          <text x="40" y="44" text-anchor="middle" font-size="5.5" font-weight="bold" fill="#FA9549">${item.input.substring(0, 16)}</text>
        </g>

        <line x1="90" y1="26" x2="120" y2="26" stroke="#FA9549" stroke-width="2" stroke-dasharray="3 2" />
    ...
    return `
      <g transform="translate(1275, ${y})">
        ...
        <line x1="295" y1="26" x2="325" y2="26" stroke="#FA9549" stroke-width="2" stroke-dasharray="3 2" />
...
window.togglePauseSync = function() {
  ...
  if (window.isSyncPaused) {
    ...
    btn.style.color = "#fff";
    document.getElementById("save-status-text").innerText = "Pausado - " + (window.liveMetrics?.active_save_file || "");
  } else {
...
window.toggleSaveDropdown = async function() {
  ...
  dd.innerHTML = "<div style='padding:8px; color:#888;'>Cargando partidas...</div>";
  try {
    const res = await fetch("/api/saves?t=" + Date.now()).catch(()=>null);
    if (res && res.ok) {
      const saves = await res.json();
      if (saves.length > 0) {
        dd.innerHTML = saves.map(s => `<div style="padding:8px; border-bottom:1px solid #333; cursor:pointer;" onclick="selectSaveFile('${s.name || s}'); event.stopPropagation();" onmouseover="this.style.background='#333'" onmouseout="this.style.background=''">${s.name || s}</div>`).join("");
      } else {
        dd.innerHTML = "<div style='padding:8px; color:#888;'>No hay partidas locales.</div>";
      }
    } else {
      dd.innerHTML = "<div style='padding:8px; color:#888;'>No disponible (Servidor Filebrowser).</div>";
    }
  } catch(e) {
    dd.innerHTML = "<div style='padding:8px; color:#888;'>Error al cargar.</div>";
  }
};

window.selectSaveFile = async function(name) {
  document.getElementById("save-dropdown").style.display = "none";
  const badge = document.getElementById("save-status-text");
  if (badge) badge.innerHTML = `<span style="color:#aaa;">⏸ Cargando ${name}...</span>`;
  ...
  try {
    ...
    // Forzar actualización visual a los 3 segundos
    setTimeout(() => {
      forceReloadLatestSave();
    }, 3000);
  } catch (err) {
    alert("Error cambiando de partida: " + err.message);
  }
};

document.addEventListener("click", e => {
  const dd = document.getElementById("save-dropdown");
  if (dd && dd.style.display === "block" && !e.target.closest("#save-status-badge") && !e.target.closest("#save-dropdown")) {
    dd.style.display = "none";
  }
});
